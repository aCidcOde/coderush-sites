    <section id="funcionalidades" class="scroll-mt-28 py-12">
      <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 class="font-[var(--font-heading)] text-3xl font-semibold sm:text-[42px]">Funcionalidades</h2>
          <div class="mt-2 h-1 w-[72px] rounded-full bg-white"></div>
          <p class="mt-4 max-w-2xl text-sm text-white/85 sm:text-base">Tudo que sua operação precisa para vender mais, reduzir retrabalho e controlar a rede em tempo real.</p>
        </div>
        <a href="#contato" class="inline-flex rounded-full border border-white/70 px-5 py-2.5 text-sm font-semibold uppercase tracking-wide hover:bg-white/10">Quero ver na prática</a>
      </div>

      <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <article data-feature-card class="group relative overflow-hidden rounded-3xl border border-white/25 bg-white/[0.06] p-5 transition duration-300 hover:-translate-y-1 hover:bg-white/[0.10] hover:shadow-[0_14px_28px_rgba(0,0,0,0.2)] focus-within:-translate-y-1">
            <div class="absolute inset-x-0 top-0 h-px bg-white/35"></div>
            <div class="flex h-full flex-col">
              <div class="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/25">
                <img decoding="async" src="imagens/pagamento-online.webp" alt="Integração com Meios de Pagamento" class="h-9 w-9" width="512" height="512" loading="lazy" />
              </div>
              <h3 class="mt-4 font-[var(--font-heading)] text-lg font-semibold">Meios de Pagamento</h3>
              <p class="mt-2 text-sm text-white/85">Conecte múltiplas opções de cobrança para reduzir fricção no checkout e aumentar conversão.</p>
              <div class="mt-3 flex flex-wrap gap-2 text-[11px] text-white/80">
                <span class="rounded-full border border-white/25 px-2 py-1">PIX</span><span class="rounded-full border border-white/25 px-2 py-1">Cartão</span><span class="rounded-full border border-white/25 px-2 py-1">Boleto</span>
              </div>
            </div>
          </article>

          <article data-feature-card class="group relative overflow-hidden rounded-3xl border border-white/25 bg-white/[0.06] p-5 transition duration-300 hover:-translate-y-1 hover:bg-white/[0.10] hover:shadow-[0_14px_28px_rgba(0,0,0,0.2)] focus-within:-translate-y-1">
            <div class="absolute inset-x-0 top-0 h-px bg-white/35"></div>
            <div class="flex h-full flex-col">
              <div class="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/25">
                <img decoding="async" src="imagens/erp.webp" alt="Integração com ERP" class="h-9 w-9" width="128" height="128" loading="lazy" />
              </div>
              <h3 class="mt-4 font-[var(--font-heading)] text-lg font-semibold">Integração com ERP</h3>
              <p class="mt-2 text-sm text-white/85">Sincronize produtos, pedidos e financeiro para eliminar retrabalho e manter dados consistentes.</p>
              <div class="mt-3 flex flex-wrap gap-2 text-[11px] text-white/80">
                <span class="rounded-full border border-white/25 px-2 py-1">Estoque</span><span class="rounded-full border border-white/25 px-2 py-1">Fiscal</span><span class="rounded-full border border-white/25 px-2 py-1">Pedidos</span>
              </div>
            </div>
          </article>

          <article data-feature-card class="group relative overflow-hidden rounded-3xl border border-white/25 bg-white/[0.06] p-5 transition duration-300 hover:-translate-y-1 hover:bg-white/[0.10] hover:shadow-[0_14px_28px_rgba(0,0,0,0.2)] focus-within:-translate-y-1">
            <div class="absolute inset-x-0 top-0 h-px bg-white/35"></div>
            <div class="flex h-full flex-col">
              <div class="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/25">
                <img decoding="async" src="imagens/no-mundo-todo.webp" alt="Integração com Logística" class="h-9 w-9" width="512" height="512" loading="lazy" />
              </div>
              <h3 class="mt-4 font-[var(--font-heading)] text-lg font-semibold">Logística Integrada</h3>
              <p class="mt-2 text-sm text-white/85">Automatize envio, rastreio e acompanhamento de entregas com mais previsibilidade operacional.</p>
              <div class="mt-3 flex flex-wrap gap-2 text-[11px] text-white/80">
                <span class="rounded-full border border-white/25 px-2 py-1">Frete</span><span class="rounded-full border border-white/25 px-2 py-1">Rastreio</span><span class="rounded-full border border-white/25 px-2 py-1">Expedição</span>
              </div>
            </div>
          </article>

          <article data-feature-card class="group relative overflow-hidden rounded-3xl border border-white/25 bg-white/[0.06] p-5 transition duration-300 hover:-translate-y-1 hover:bg-white/[0.10] hover:shadow-[0_14px_28px_rgba(0,0,0,0.2)] focus-within:-translate-y-1">
            <div class="absolute inset-x-0 top-0 h-px bg-white/35"></div>
            <div class="flex h-full flex-col">
              <div class="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/25">
                <img decoding="async" src="imagens/grafico-de-barras.webp" alt="Relatórios Completos" class="h-9 w-9" width="512" height="512" loading="lazy" />
              </div>
              <h3 class="mt-4 font-[var(--font-heading)] text-lg font-semibold">Relatórios Completos</h3>
              <p class="mt-2 text-sm text-white/85">Acompanhe indicadores de rede e finanças para decisões mais rápidas e orientadas por dados.</p>
              <div class="mt-3 flex flex-wrap gap-2 text-[11px] text-white/80">
                <span class="rounded-full border border-white/25 px-2 py-1">Rede</span><span class="rounded-full border border-white/25 px-2 py-1">Financeiro</span><span class="rounded-full border border-white/25 px-2 py-1">Performance</span>
              </div>
            </div>
          </article>

          <article data-feature-card class="group relative overflow-hidden rounded-3xl border border-white/25 bg-white/[0.06] p-5 transition duration-300 hover:-translate-y-1 hover:bg-white/[0.10] hover:shadow-[0_14px_28px_rgba(0,0,0,0.2)] focus-within:-translate-y-1">
            <div class="absolute inset-x-0 top-0 h-px bg-white/35"></div>
            <div class="flex h-full flex-col">
              <div class="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/25">
                <img decoding="async" src="imagens/mapa-do-site.webp" alt="Plano de Compensação" class="h-9 w-9" width="512" height="512" loading="lazy" />
              </div>
              <h3 class="mt-4 font-[var(--font-heading)] text-lg font-semibold">Plano de Compensação</h3>
              <p class="mt-2 text-sm text-white/85">Modele regras de bônus e comissionamento conforme sua estratégia comercial e fase de crescimento.</p>
              <div class="mt-3 flex flex-wrap gap-2 text-[11px] text-white/80">
                <span class="rounded-full border border-white/25 px-2 py-1">Binário</span><span class="rounded-full border border-white/25 px-2 py-1">Unilevel</span><span class="rounded-full border border-white/25 px-2 py-1">Custom</span>
              </div>
            </div>
          </article>

          <article data-feature-card class="group relative overflow-hidden rounded-3xl border border-white/25 bg-white/[0.06] p-5 transition duration-300 hover:-translate-y-1 hover:bg-white/[0.10] hover:shadow-[0_14px_28px_rgba(0,0,0,0.2)] focus-within:-translate-y-1">
            <div class="absolute inset-x-0 top-0 h-px bg-white/35"></div>
            <div class="flex h-full flex-col">
              <div class="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/25">
                <img decoding="async" src="imagens/trabalho-em-equipe.webp" alt="Acesso mobile" class="h-9 w-9" width="512" height="512" loading="lazy" />
              </div>
              <h3 class="mt-4 font-[var(--font-heading)] text-lg font-semibold">Acesso Mobile</h3>
              <p class="mt-2 text-sm text-white/85">Acompanhe e opere a rede de qualquer lugar com experiência responsiva para time e distribuidores.</p>
              <div class="mt-3 flex flex-wrap gap-2 text-[11px] text-white/80">
                <span class="rounded-full border border-white/25 px-2 py-1">Responsivo</span><span class="rounded-full border border-white/25 px-2 py-1">Rápido</span><span class="rounded-full border border-white/25 px-2 py-1">Prático</span>
              </div>
            </div>
          </article>
      </div>
    </section>
